...
function stripHtml(html: string): string {
    ...
    // Simple truncation (be careful not to break tags, so we just limit the raw input strings if they are too massive, 
    // but typically safe HTML is fine, we'll cap to 15000 chars)
...
export class ScraperService {
    ...
    private extractTechSkills(text: string): string[] {
        ...
        for (const kw of techKeywords) {
            ...
            // \b is not reliable for non-words like ++ or .js, so we use (^|\s|\W)
    ...
    async scrapeRemotive(): Promise<RawJob[]> {
        ...
        try {
            ...
            // Fetch software-dev, react, frontend, fullstack
            const queries = [
